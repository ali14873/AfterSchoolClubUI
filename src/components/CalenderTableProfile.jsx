...
const CalenderTableProfile = ({ idss }) => {
  const [data, setdata] = useState([]);

  const [sessions, setSessions] = useState([]);

  const fetchSessions = async (userIds) => {
    try {
      const sessionData = [];

      // Iterate over each user ID
      for (const userId of userIds) {
        // Fetch sessions for the current user ID
        const response = await axios.get(
          `https://afterschoolclub.azurewebsites.net/sessions/get/${userId}`
        );
        sessionData.push(...response.data);
      }

      // Manage or combine session data as needed
  ...
  const fetchStudents = async () => {
    try {
      const response = await axios.get(
        `https://afterschoolclub.azurewebsites.net/sessionBookings/byUser/${idss}`
      );
      const userIds = response.data.map((student) => student.userId); // Assuming user IDs are under 'userId' key, adjust accordingly

      const sessions = await fetchSessions(userIds);
      setSessions(sessions); // Assuming you have a state to store session data
  ...
  const fetchSession = async () => {
    try {
      ...
      const sessionBookings = response.data; // Assuming session bookings are returned directly in the response

      // Fetch details for each session booking

      console.log(sessionBookings);
      const sessionDetailsPromises = sessionBookings.map(
        async (sessionBooking) => {
          try {
            const sessionDetailsResponse = await axios.get(
              `https://afterschoolclub.azurewebsites.net/sessions/get/${sessionBooking.sessionID}`
            );

            console.log("SessionDetailsData", sessionDetailsResponse.data);

            return sessionDetailsResponse.data;
          } catch (error) {
            console.error(
              `Error fetching session details for session ID ${sessionBooking.sessionID}:`,
              error.message
            );
            return null;
          }
      ...
      setSortedData(sessionDetails);
    } catch (error) {
      console.error("Error fetching session bookings:", error.message);
      toast.error("Failed to fetch session bookings");
    }
  };
  ...
  const handleSearch = (event) => {
    ...
    const filteredData = data.filter(
      (row) =>
        ...
        row.start.toLowerCase().includes(value) ||
        row.end.toLowerCase().includes(value) ||
        row.description.toLowerCase().includes(value) ||
        row.location.toLowerCase().includes(value) ||
        row.clubID.toLowerCase().includes(value) ||
        row.isRecurring.toLowerCase().includes(value) ||
        row.recurrenceRule.toLowerCase().includes(value)
    );
  ...
  const handleOpenModalChildrenSession = (id) => {
      ...
      <Table className="mt-4 w-100 mx-auto  " responsive>
        <thead>
          <tr>
            ...
            {(logined === "user" && selectedRows && selectedRows.length <= 1) ||
            logined === "admin" ? (
              <th>Action</th>
            ) : (
              ""
...
export default CalenderTableProfile;

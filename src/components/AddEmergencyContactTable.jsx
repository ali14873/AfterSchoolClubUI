...
import EditModalEmergencyContact from "./EditModalEmergencyContact";
import axios from "axios";
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

const AddEmergencyContactTable = ({ none, idss }) => {
  ...
  useEffect(() => {
    if (idss) {
      ...
      console.log("Fetching Without Id");
    }
  }, [idss]);
  ...
  const fetchEmergencyContact = async () => {
    try {
      const response = await axios.get(
        "https://afterschoolclub.azurewebsites.net/emergencyContact/getAll"
      );
  ...
  const fetchEmergencyContactbyid = async () => {
    try {

      console.log(idss);
      const response = await axios.get(
        `https://afterschoolclub.azurewebsites.net/emergencyContact/getByStudentID/${idss}`
      );
      setdata(response.data);
      setSortedData(response.data);
      console.log(response.data)
  ...
  const handleSearch = (event) => {
    ...
    const filteredData = data ? data.filter(
      (row) =>
        ...
        row.relationship.toLowerCase().includes(value) ||
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are You Sure You Want to Delete?")) {
      try {
        await axios.delete(
          `https://afterschoolclub.azurewebsites.net/emergencyContact/delete/${id}`
        );
      } catch (error) {
...
export default AddEmergencyContactTable;
